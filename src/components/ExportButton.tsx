...
export function ExportButton<T, F>({ service, filters, fileName, token, columns }: ExportButtonProps<T, F>) {
  const handleExport = async () => {
    try {
      toast.info(`Exporting ${fileName} data... This may take a moment.`);

      // Fetch all data with a large limit
      const response = await service.findAll(token, { ...filters, limit: 1000000, page: 1 });
      const dataToExport: T[] = response.data;

      console.table(dataToExport[0])

      if (!dataToExport || dataToExport.length === 0) {
        toast.warning(`No ${fileName} data found to export with the current filters.`);
        return;
      }
      ...
      const ws_data = [
        ...
        ...dataToExport.map((row:any) => columns.map(col => col.accessor === "location" ? row?.["location"]?.name : row[col.accessor] ?? "N/A")), // Data rows
      ];
