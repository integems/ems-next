...
> = {
  ph: { unit: "", name: "pH" },
  nitrogen: { unit: "mg/kg", name: "Nitrogen" },
  phosphorus: { unit: "mg/kg", name: "Phosphorus" },
  potassium: { unit: "mg/kg", name: "Potassium" },
  organicMatter: { unit: "%", name: "Organic Matter" },
  moisture: { unit: "%", name: "Moisture" },
};
...
interface SoilAIChatAnalysisProps {
  soilData: PaginationResponse<SoilData>;
...
export default function SoilAIChatAnalysis({
  ...
  const groupedData = useMemo(() => {
    return filteredSoilData.reduce(
      (acc, item) => {
  ...
  const timePeriods = useMemo(() => {
    if (filteredSoilData.length === 0) return [];

    const periodMap = new Map<string, Date>();

    filteredSoilData.forEach((item) => {
  ...
  const buildReport = () =>
    buildAnalysisReport({
      ...
      fileBaseName: "soil-analysis-report",
      ...
      timeSeriesData,
      groupedData,
      filteredCount: filteredSoilData.length,
      aiText,
