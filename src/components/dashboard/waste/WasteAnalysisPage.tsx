...
import { WasteData, Location, LocationType } from "@/types/common.types";
...
> = {
  solidWasteKg: { unit: "Kg", name: "Solid Waste" },
  hazardousWasteKg: { unit: "Kg", name: "Hazardous Waste" },
  recycledWasteKg: { unit: "Kg", name: "Recycled Waste" },
  organicWasteKg: { unit: "Kg", name: "Organic Waste" },
  plasticWasteKg: { unit: "Kg", name: "Plastic Waste" },
  paperWasteKg: { unit: "Kg", name: "Paper Waste" },
  cansWasteKg: { unit: "Kg", name: "Cans Waste" },
  bottlesWasteKg: { unit: "Kg", name: "Bottles Waste" },
  eWasteKg: { unit: "Kg", name: "E-Waste" },
  scrapMetalKg: { unit: "Kg", name: "Scrap Metal" },
};
...
export default function WasteAnalysisPage() {
  ...
  //     return wasteData.filter(item => item.locationId && locationIdsFilter.includes(item.locationId));
  //   }
  //   return wasteData;
  // }, [wasteData, locationIdsFilter]);
  ...
  const timeSeriesData = useMemo(() => {
    if (!wasteData || !timePeriods.length) return [];
  ...
  }, [wasteData, timePeriods, groupedData, locations, selectedParameter, chartType]);

  // Prepare data for bar chart
  const barChartData = useMemo(() => {
    if (!wasteData || !timePeriods.length || !locationIdsFilter.length) return [];
  ...
  }, [wasteData, timePeriods, groupedData, locations, selectedParameter, chartType, locationIdsFilter]);
            ...
            <div className="w-full ">
               <label className="block text-sm font-medium mb-2">Paramter Selection</label>
                <Select
                  ...
                  onValueChange={(value) => setSelectedParameter(value as keyof WasteData)}
                >
