...
import { WaterData, Location, LocationType, WaterSource } from "@/types/common.types";
...
> = {
  ph: { unit: "", name: "pH" },
  phMv: { unit: "mV", name: "pH (mV)" },
  orp: { unit: "mV", name: "ORP" },
  ec: { unit: "µS/cm", name: "EC" },
  ecAbs: { unit: "µS/cm", name: "Absolute EC" },
  resistivity: { unit: "kΩ·cm", name: "Resistivity" },
  salinity: { unit: "PSU", name: "Salinity" },
  pressure: { unit: "mbar", name: "Pressure" },
  doPercent: { unit: "%", name: "DO (%)" },
  dissolvedOxygen: { unit: "mg/L", name: "Dissolved Oxygen" },
  turbidity: { unit: "NTU", name: "Turbidity" },
  bod: { unit: "mg/L", name: "BOD" },
  cod: { unit: "mg/L", name: "COD" },
  totalDissolvedSolids: { unit: "mg/L", name: "TDS" },
...
export default function WaterAnalysisPage() {
  ...
  >(undefined);
  const [waterSourceFilter, setWaterSourceFilter] = useState<"surface" | "underground" | "All" | undefined>(undefined);
  ...
  //     return waterData.filter(item => item.locationId && locationIdsFilter.includes(item.locationId));
  //   }
  //   return waterData;
  // }, [waterData, locationIdsFilter]);
  ...
  const timeSeriesData = useMemo(() => {
    if (!waterData || !timePeriods.length) return [];
  ...
  }, [waterData, timePeriods, groupedData, locations, selectedParameter, chartType]);

  // Prepare data for bar chart
  const barChartData = useMemo(() => {
    if (!waterData || !timePeriods.length || !locationIdsFilter.length) return [];
  ...
  }, [waterData, timePeriods, groupedData, locations, selectedParameter, chartType, locationIdsFilter]);
  ...
  const parameterOptions: { value: keyof WaterData; label: string }[] = [
    ...
    { value: "ecAbs", label: "Absolute EC (µS/cm)" },
    { value: "resistivity", label: "Resistivity (kΩ·cm)" },
    { value: "salinity", label: "Salinity (PSU)" },
    { value: "pressure", label: "Pressure (mbar)" },
    { value: "doPercent", label: "DO (%)" },
    ...
    { value: "totalDissolvedSolids", label: "TDS (mg/L)" },
      ...
      <div className="py-8 space-y-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 items-end">
          ...
          <div className="flex-1">
            ...
            <Select
              value={waterSourceFilter || "All"}
              onValueChange={(value) =>
                setWaterSourceFilter(
                  value === "All"
                    ? undefined
                    : (value as "surface" | "underground")
                )
              }
            >
              <SelectTrigger className="bg-background border-border">
            ...
            <div className="w-full ">
               <label className="block text-sm font-medium mb-2">Paramter Selection</label>
                <Select
                  ...
                  onValueChange={(value) => setSelectedParameter(value as keyof WaterData)}
                >
