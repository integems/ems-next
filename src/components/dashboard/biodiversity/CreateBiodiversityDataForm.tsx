...
export default function CreateBiodiversityDataForm({
  ...
  const pendingEntriesRef = useRef<BiodiversityDataFormData[]>([]);
  ...
  const createBiodiversityDataMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      toast.success("Biodiversity data added successfully!");
      onClose();
    },
    onError: (error) => {
  ...
  const createLocationMutation = useMutation({
    ...
    onSuccess: (data) => {
      ...
      const biodiversityDataWithLocation = pendingEntriesRef.current.map(
        (data) => ({
          ...data,
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    let entries = biodiversityDataFormData;
    if (entries.length === 0) {
      const parsed = updatedBiodiversityDto.safeParse(singleBiodiversityData);
      if (parsed.success) {
        entries = [parsed.data];
        setBiodiversityDataFormData(entries);
      } else {
        setErrors(parsed.error.flatten().fieldErrors);
        toast.error(
          "Add at least one biodiversity data entry before submitting.",
        );
        return;
      }
    }
    ...
    if (isCreatingLocation) {
      ...
      const biodiversityDataWithLocation = entries.map((data) => ({
        ...data,
      ...
      <form onSubmit={handleSubmit} className="flex flex-col">
        ...
        <div className="flex items-center justify-end gap-3 pt-6 border-t mt-6">
          <span className="mr-auto text-xs text-muted-foreground">
            {biodiversityDataFormData.length > 0
              ? `${biodiversityDataFormData.length} entr${
                  biodiversityDataFormData.length === 1 ? "y" : "ies"
                } ready to submit`
