...
export default function BiodiversityAIChatAnalysis({
  ...
  const filteredBiodiversityData = useMemo(() => {
    let data = biodiversityData.data;
  ...
  const allLocations = useMemo(() => {
    ...
    biodiversityData.data.forEach((data) => {
  ...
  const displayedLocations = useMemo(() => {
    const uniqueLocations = new Map<string, Location>();
    filteredBiodiversityData.forEach((data) => {
      if (data.location && !uniqueLocations.has(data.location.locationId)) {
        uniqueLocations.set(data.location.locationId, data.location);
      }
    });
    return Array.from(uniqueLocations.values());
  }, [filteredBiodiversityData]);

  const groupedData = useMemo(() => {
    return filteredBiodiversityData.reduce(
      (acc, item) => {
  ...
  const numericData = useMemo(() => {
    return filteredBiodiversityData
      .map((item) => Number(item[selectedParameter]))
      .filter((v) => !isNaN(v) && v !== null && v !== undefined);
  }, [filteredBiodiversityData, selectedParameter]);
  ...
  const timePeriods = useMemo(() => {
    if (filteredBiodiversityData.length === 0) return [];

    const periodMap = new Map<string, Date>();

    filteredBiodiversityData.forEach((item) => {
  ...
  }, [filteredBiodiversityData, chartType]);
