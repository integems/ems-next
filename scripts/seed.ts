  ...
  try {
    ...
    for (const city of cities) {
      ...
      await db.insert(schema.locations).values({
        ...
        locationType:
          locationTypes[Math.floor(Math.random() * locationTypes.length)] as any,
        createdBy: "system",
        createdAt: now,
        updatedBy: "system",
        updatedAt: now,
    ...
    const categories = ["air", "water", "soil", "noise", "biodiversity", "waste"];
    ...
    for (const cityName in locations) {
      ...
      for (let i = 0; i < 12; i++) { // For each of the last 12 months
        const measurementTime = new Date(now.getFullYear(), now.getMonth() - i, Math.floor(Math.random() * 28) + 1);
        ...
        for (const category of categories) {
            ...
            if (category === "air") {
              ...
              await db.insert(schema.waterData).values({
                ...
                waterSource:
                  waterSources[Math.floor(Math.random() * waterSources.length)] as any,
